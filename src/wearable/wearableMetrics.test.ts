...
function record(id:string,date:string,payload:Record<string,unknown>):RawRecord{return{id,record_type:'experiment',created_at:`${date}T12:00:00Z`,payload:{version:'1.0',subtype:'wearable_daily',date,source:'Garmin',...payload}}}
...
describe('wearable import',()=>{
  ...
  it('parses semicolon-delimited CSV',()=>{const rows=parseWearableText('date;sleep_minutes;resting_bpm\n2026-08-20;420;61','Garmin');expect(rows[0]).toMatchObject({sleepMinutes:420,restingBpm:61})})
...
describe('wearable longitudinal summary',()=>{
  ...
  it('does not mix baselines from another source',()=>{const other=[1,2,3,4,5].map(day=>({id:`s${day}`,record_type:'experiment',created_at:`2026-08-0${day}T12:00:00Z`,payload:{version:'1.0',subtype:'wearable_daily',date:`2026-08-0${day}`,source:'Samsung',sleepMinutes:200}} as RawRecord));const summary=summarizeWearable([...other,...baseline,record('x','2026-08-10',{sleepMinutes:390})],'2026-08-10');expect(summary.sleep.baseline).toBe(450)})
