...
export default function WearableImport({onImported}:{onImported:()=>Promise<void>|void}){
  ...
  const choose=async(file:File|null)=>{
    ...
    try{
      ...
      setPreview(rows)
      setMessage(`${rows.length} dia(s) reconhecido(s). Revise a fonte e confirme a importação.`)
  ...
  const save=async()=>{
    ...
    try{
      ...
      const{data:existing,error:existingError}=await supabase.from('nexo_records').select('payload').eq('record_type','review')
      ...
      for(let start=0;start<rows.length;start+=200){
        const batch=rows.slice(start,start+200).map(payload=>({user_id:user.id,record_type:'review',payload}))
  ...
  return <div className="wearable-import">
    <div className="wearable-import-grid">
      <label>Fonte do relógio ou app<input value={source} maxLength={80} placeholder="Ex.: Garmin, Samsung Health" onChange={event=>setSource(event.target.value||'smartwatch')}/></label>
